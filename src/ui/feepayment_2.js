...
const student_no = receivedData['student_no'];

async function init(student_no) {

  const result = await main.getStudentbyNumber(student_no);
  console.log(result);

  const payments =  await main.paymentsbystudent(student_no);
...
feeDetail.addEventListener("submit", async (e) => {
  try {
    ...
    const result = await main.getStudentbyNumber(student_no);
    const feetypes = await main.getFeesofClass(result.class);
